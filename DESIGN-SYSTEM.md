...
Оформление выросло из приложения «Распаковка конкурентов» (приватный репозиторий
`christophermaasi/unpack-competitors`, папка
`ТЕСТ-веб-приложение/3-скрипт-веб-приложения-новый/`): `Index.html` — каркас,
токены и общие помощники, `Dashboard.html` — сетка таблиц, дальше `Card.html`,
`Queries.html`, `Heatmap.html`. Всё, что нужно для переноса, собрано здесь;
ходить в исходный проект не требуется.
